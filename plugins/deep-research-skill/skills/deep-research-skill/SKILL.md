...
**Текущий статус (v1.5.0 / Phase 15 + i18n + simplification):** движок Phase 1–15 реализован. **v1.4.0:** отчёт рендерится на языке из `TaskFrame.language` (`ru` по умолчанию, `en` поддержан) — ранее парсившееся, но неиспользуемое поле теперь управляет языком отчёта end-to-end (инвариант #9). **v1.5.0 (simplification):** удалены инертные фичи — anti-fit veto-слой (срабатывал лишь на `.example`-доменах) и in-pipeline auto-verify (писал `claim.metadata["disagreement"]`, в отчёт не рендерился); отчёт стал лин по умолчанию (confidence-эмодзи + per-source score-breakdown — по `--verbose`). Метрики доверия не пострадали (trust-scorecard неизменен); см. `docs/PHASE_V1.5_SIMPLIFICATION_PLAN.md`, инвариант #10 «shipped ≠ valuable». Phase 15 «Integration & Reachability» устраняет разрыв между реализованными возможностями и CLI: добавлены три ранее недоступных через CLI глагола — `verify` (независимая ре-деривация категорий утверждений), `plan` (валидация DAG, топологический порядок, ready-set) и `gate` (статус блокирующих gate-сигналов); `snapshot.trace` (список `{event, ts}` словарей из `RunTrace.as_list()`) наполняется событием на каждой стадии пайплайна (стадии: ingest/score/factcheck/cluster/build — in-pipeline auto-verify убран в v1.5, остался standalone-глагол `verify`). CHECKPOINT_VERSION повышена до "1.3". Phase 14 «Explainable & Calibratable Scoring» добавила anti-fit veto-слой (`disqualify` → composite=0 / tier=D; **убран в v1.5** как инертный), аудит-трейс скора (`scores.breakdown` — `[label, contribution]` суммой в composite), CLI-подкоманду `rescore` (полная ре-деривация над замороженным read-only набором, `--shallow` опция), append-only feedback-ledger (`memory --op record-feedback|list-feedback`, НЕ авто-консумится скорингом) и дешёвый RSS-тир сбора (`collect` provider=`rss`, `published_at`→`DateConfidence.HIGH`) с расширенным словарём `next_valid_actions`. Phase 13 «Cost & Cache» добавила захват cache-сигналов в телеметрию (`cache_hit_rate`, `bundle_hash`, `cache_fragmentation`), именованные границы компактизации (`should_compact`), CLI-подкоманду `cost` и cost/latency-регрессию — см. секцию «Prompt Caching & Stable Prefix». Phase 9 «Typed Collection Seam» добавила `engine/collect.py` — единый типизированный контракт результата сбора `{status, summary, items[], next_valid_actions[]}` над любым провайдером (web_search/Jina/Firecrawl/RSS/…), со snippet-cap, in-session URL-дедупом и risk_class на провайдер. Phase 8 «Trust & Grounding» — trust-fence на retrieved-контент (prompt-injection защита), machine-readable remediation в fact-check и adversarial-тесты. **Все CLI-подкоманды функциональны** (JSON-in/JSON-out): `run`, `collect`, `report`, `ingest`, `rank`, `score`, `factcheck`, `verify` (новая), `plan` (новая), `gate` (новая), `cluster`, `memory`, `eval`, `cost`, `rescore`, `checkpoint`, `resume`, `compact`, `hook`, `doctor`. Без Python / code-execution — prose-only режим. Дорожная карта — `docs/REBUILD_PLAN.md`.

---

# Deep Research Skill

> **Версия:** 1.5.0 | **Последнее обновление:** 2026-06-15
...
Phase 14 added a per-source **score breakdown** that makes every composite score fully auditable. (Phase 14 also shipped an **anti-fit VETO layer**, removed in v1.5 — see below.)

### Anti-fit VETO layer — removed in v1.5

> **Status: removed in v1.5.** The veto layer (`VetoRules` / `DEFAULT_VETO` / `disqualify`) was cut as inert — its seed rules matched only a handful of `.example` test hosts and two self-declaring phrases, moving no DRACO score and no trust metric (trust-scorecard byte-identical before/after; invariant #10 «shipped ≠ valuable»). Prompt-injection defense is unaffected — it is provided by the ingest trust-fence (Phase 8), not the veto. The `Source.scores.disqualifiers` field is retained for checkpoint backward-compat and is now always empty (old checkpoints with legacy veto reasons still round-trip). `score_source` no longer takes a `VetoRules` argument.
...
**Report surface.** The breakdown is opt-in via `--verbose` (v1.5: the report is lean by default). With `--verbose`, each scored source in the source registry (the Источники / Sources section) shows its `composite`, `tier`, and the `breakdown` trace. The lean default keeps `[Sx]` + tier + URL.
...
*Deep Research Skill v1.5.0 — testing release. Полная документация: SKILL.master.md. Язык адаптируется к языку пользователя. All phase modules load on-demand — no eager loading.*
