...
> **Версия:** 2.0.0 | **Последнее обновление:** 2026-06-07
...
Heartbeat каждые 30 сек + checkpoint после каждого phase gate. При прерывании — восстановление из последнего checkpoint. Никакой потери прогресса. Recovery protocol: read heartbeat → identify last checkpoint → restore state → resume from next gate.
...
*Deep Research Skill v2.0.0 — production-ready. 18/18 Acceptance Criteria implemented. Полная документация: SKILL.master.md. Язык адаптируется к языку пользователя. All phase modules load on-demand — no eager loading.*
